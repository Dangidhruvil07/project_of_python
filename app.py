...
def add_to_cart(id):
    ...
    item = mongo.db.cart.find_one({"name": product["name"], "email": session["user"]})

    if item:
        mongo.db.cart.update_one({"_id": item["_id"]}, {"$inc": {"quantity": 1}})
    else:
...
def checkout():
    ...
    if cart_items:
        ...
        mongo.db.orders.insert_one(
            {"email": session["user"], "items": cart_items, "total": total}
        )
...
def myorders():
    ...
    orders = mongo.db.orders.find({"email": session["user"]}).sort("_id", -1)
